...
function withTeam(name = "Lakers", playerCount = 3) {
  const state = reducer(
    undefined,
    teamCreated({ name, playerCount, region: "West", country: "USA" }),
  );
...
describe("teams", () => {
  it("creates a team and keeps display order", () => {
    ...
    expect(state.order.map((id) => state.teams[id]?.name)).toEqual([
      "Alpha",
      "Beta",
    ]);
  });
...
describe("player assignment", () => {
  it("assigns a player to a team", () => {
    ...
    const next = reducer(
      state,
      playerAssigned({ teamId: id, player: player(1) }),
    );
  ...
  it("never lets a player belong to two teams", () => {
    let state = reducer(
      undefined,
      teamCreated({ name: "A", playerCount: 5, region: "W", country: "USA" }),
    );
    state = reducer(
      state,
      teamCreated({ name: "B", playerCount: 5, region: "E", country: "USA" }),
    );
    const [first, second] = state.order as [string, string];

    state = reducer(
      state,
      playerAssigned({ teamId: first, player: player(7) }),
    );
    state = reducer(
      state,
      playerAssigned({ teamId: second, player: player(7) }),
    );
  ...
  it("ignores assignment to a team that does not exist", () => {
    const next = reducer(
      undefined,
      playerAssigned({ teamId: "ghost", player: player(1) }),
    );
  ...
  it("stores a full player snapshot so rosters survive a reload", () => {
    ...
    const next = reducer(
      state,
      playerAssigned({ teamId: id, player: player(42) }),
    );
...
describe("deleting a team", () => {
  it("releases its players but leaves other teams untouched", () => {
    ...
    state = reducer(
      state,
      playerAssigned({ teamId: first, player: player(1) }),
    );
    state = reducer(
      state,
      playerAssigned({ teamId: second, player: player(2) }),
    );
  ...
  it("lets a released player join another team afterwards", () => {
    ...
    state = reducer(
      state,
      playerAssigned({ teamId: first, player: player(9) }),
    );
    state = reducer(state, teamDeleted(first));
    state = reducer(
      state,
      playerAssigned({ teamId: second, player: player(9) }),
    );
